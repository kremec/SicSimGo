...
func Disassembly(gtx *layout.Context, theme *material.Theme, instructionList *widget.List) layout.Dimensions {
	return layout.Flex{
		...
		layout.Flexed(1, func(gtx C) D {
			return material.List(theme, instructionList).Layout(gtx, len(loader.Instructions), func(gtx C, index int) D {
				instruction := loader.Instructions[index]
				...
				if instruction.Instruction.Format == proc.InstructionFormat2 {
					instructionOperand = fmt.Sprintf("%s,%s", instruction.R1.String(), instruction.R2.String())
				} else if instruction.Instruction.Opcode.IsJumpInstruction() {
